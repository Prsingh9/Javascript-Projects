//create an array containing paragraphs as elements
...
form.addEventListener("submit",function(e){ //event object, function is triggered on clicking submit button
    // console.log("demo");
    //we click on form submit button , it initiates its submission to the server

    e.preventDefault();//this will prevent the default behaviour of form that is sending data to the server

    const value=parseInt(amount.value) ; //this is the form input value
    // console.log(typeof value);

    const random= Math.floor(Math.random() * text.length) ;// this will generate random numbers between 0 to 8

    if(isNaN(value) || value<=0 || value>9){
        result.innerHTML=` <p class="result">${text[random]}</p>`;
    }
    else{
        let tempText=text.slice(0,value);// returns a new array , suppose value=2 , it will return array with two paragraph
